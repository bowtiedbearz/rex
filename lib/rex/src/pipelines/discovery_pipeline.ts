...
import { LoggingMessageBus } from "../message-bus/bus.ts";
import type { ExecutionContext } from "./contexts.ts";
import { Job, JobMap } from "./jobs/primitives.ts";
import { Next, Pipeline } from "./pipeline.ts";
import { Task, TaskMap } from "./tasks/primitives.ts";
...
try {
    ...
    if (!mod.tasks) {
        ...
        if (!(mod.tasks instanceof TaskMap)) {
            ctx.bus.error(new Error(`Tasks in ${file} must be of type TaskMap`), 'Error collecting tasks');
            return;
        }
    ...
    if (!mod.jobs) {
        if (globalJobs.size === 0) {
            ctx.bus.debug(`No jobs found in ${file}.  Task file must export a variable called jobs is type JobMap.`);
        } else {
        ...
        if (!(mod.jobs instanceof JobMap)) {
            ctx.bus.error(new Error(`Jobs in ${file} must be of type JobMap`), 'Error collecting tasks');
            return;
        }
